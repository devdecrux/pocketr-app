...
object LedgerTxnSpecs {
    ...
    fun forUser(userId: Long): Specification<LedgerTxn> = Specification { root, _, cb ->
        cb.equal(root.get<Any>("createdBy").get<Long>("userId"), userId)
    }

    fun dateFrom(date: LocalDate): Specification<LedgerTxn> = Specification { root, _, cb ->
        cb.greaterThanOrEqualTo(root.get("txnDate"), date)
    }

    fun dateTo(date: LocalDate): Specification<LedgerTxn> = Specification { root, _, cb ->
        cb.lessThanOrEqualTo(root.get("txnDate"), date)
    }

    fun hasAccount(accountId: UUID): Specification<LedgerTxn> = Specification { root, query, cb ->
        val splits = root.join<Any, Any>("splits")
        query.distinct(true)
        cb.equal(splits.get<Any>("account").get<UUID>("id"), accountId)
    }

    fun hasCategory(categoryId: UUID): Specification<LedgerTxn> = Specification { root, query, cb ->
        val splits = root.join<Any, Any>("splits")
        query.distinct(true)
